...
class Response
{
    ...
    public function getStatus(): string
    {
        ...
        // Verifica se a propriedade "status" existe no objeto "stdClass"
        if (property_exists($this->data, 'status')) {
            // Acessa a propriedade "status" se ela existir
            $status = $this->data->status;
        } else {
            // Define um valor padrão para "status" se a propriedade não existir
            $status = 'unknown';
        }

        // Verifica se o valor de "status" é nulo e retorna uma string vazia se for
        if ($status === null) {
            return '';
        }

        // Retorna o valor de "status" como uma string
